...
const AdminClients = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axiosInstance.get("/admin-client-list");
  ...
  const toggleBlock = async (item) => {
    try {
      ...
      if (res?.data?.success) {
        Swal.fire({ title: "Client", text: res?.data?.message, icon: "success" });
        fetchData();
      } else {
        Swal.fire({ title: "Client", text: res?.data?.message, icon: "error" });
      }
    } catch (error) {
